#!/usr/bin/env python
"""Run VAD pipeline with debug to see what's happening."""
import sys, os
sys.path.insert(0, os.path.dirname(__file__))

# Remove old dataset files
for f in os.listdir("dataset_test"):
    if f.startswith("T_zgDuLSIYU"):
        os.remove(os.path.join("dataset_test", f))

from tts_pipeline.config import Settings
from tts_pipeline.downloader import download_video_and_subs, get_channel_videos
from tts_pipeline.parser import parse_vtt_full, merge_cues
from tts_pipeline.vad import get_speech_intervals
from tts_pipeline.vad_aligner import group_vad_intervals, align_text_to_groups

s = Settings()
raw = str(s.raw_dir)
out = str(s.output_dir)

print("[Step 1] Download...")
video_info = download_video_and_subs("https://www.youtube.com/watch?v=T_zgDuLSIYU", raw, s)

print("[Step 2] Parse VTT...")
raw_cues = parse_vtt_full(video_info["vtt_path"])
print(f"   Raw cues: {len(raw_cues)}")

print("[Step 3] VAD...")
speech = get_speech_intervals(
    video_info["audio_path"],
    sample_rate=s.sample_rate,
    threshold=0.3,
    min_speech_dur=0.3,
    min_silence_dur=0.3,
)
print(f"   Speech intervals: {len(speech)}")
if speech:
    total_speech = sum(iv["end"] - iv["start"] for iv in speech)
    print(f"   Total speech duration: {total_speech:.1f}s")
    print(f"   First 5: {speech[:5]}")
    print(f"   Last 5: {speech[-5:]}")
else:
    print("   WARNING: No speech detected!")
    # Fallback: print audio stats
    import librosa
    audio, sr = librosa.load(video_info["audio_path"], sr=16000, mono=True)
    print(f"   Audio duration: {len(audio)/16000:.1f}s")
    print(f"   RMS energy: { (audio**2).mean()**0.5:.4f}")
